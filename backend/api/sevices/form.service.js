...
const saveResponse = async (responseJson) => {
  try {
    ...
    const responseData = await ResponseDataModel.findOne({ id });
    if (responseData) {
      let len = Object.keys(responseData.response).length;
      responseData.response[len] = response;
    } else {
      responseData.response = { 0: response };
    }
